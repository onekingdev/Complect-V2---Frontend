...
exports.zipDownload = async event => {
	try {
		...
		const record = await readDocuments({ collection, _id });
		let location;
		files = [];
		if ( record.status === "file" ) {
			const file = [{ key: record.key, name: record.title }];
...
exports.moveToDirs = async () => {
	try {
		files = [];
		const query = { folderId: "root", status: "folder" };
		const records = await readDocuments({ collection, query });
		if ( records.length == 1 || !records.length ) {
			files = [];
		} else {
			for ( let i in records ) {
				...
				const key = "";
				files.push({ key, name, status, title, value });
				let id = records[i]._id.toString();
...
const getDir = async ( current_id, current_status, current_title, current_folderId, origin_id ) => {
	if ( current_id == origin_id ) return "";
		...
		const record = await readDocuments({ collection, _id: current_folderId });
