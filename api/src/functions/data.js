...
exports.financialReports = async event => {
	try {
		...
		let totalMonthPay, totalYearPay, totalPay, totalPayingOut, annualBudget;
		totalMonthPay = 0;
		totalPay = 0;
		totalYearPay = 0;
		totalPayingOut = 0;
		...
		if ( document.financialType === "business" ) {
			...
			const business = await readDocuments({
				collection: "business",
				_id: document.accountId
			});
			annualBudget = business.annualBudget;
		} else {
			...
			const specialist = await readDocuments({
				collection: "specialist",
				_id: document.accountId
			});
			annualBudget = specialist.annualBudget;
		}
		return response({
			httpCode: 200,
			data: {
