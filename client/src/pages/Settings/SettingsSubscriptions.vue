	...
	template(#content)
		template(v-if="userType=='business'")
			...
			div.subscription-content.grid-6(v-if="linkaccount.currentPlan" :class="planClass")
				div.col-3.plan-type
					h4 {{ document.title }}
					p(v-if="userType=='business'") {{ currentSeats }} active users
...
c-modal(title="Edit Seats" v-model="isEditPlanVisible" wide)
	template(#content)
		div.col-4.grid-6
			p  You currently have {{ currentSeats }} setas
			c-select.col-3(label="Billing Plan" :data="formOptions.payments" v-model="billingPlan")
			c-field.col-3(label="Add Setas" v-model="addSeats")
			.col-3
			c-field.col-3.align-right(label="Remove Setas" v-model="removeSeats")
			...
			try {
				if ( addSeats.value > 0 ) {
					...
					notification({
						"type": "success",
						"title": "Success",
						"message": "Seats have been updated."
					});
					...
					if ( currentSeats.value - removeSeats.value < currentSeats.value ) {
						notification({
							"type": "error",
							"title": "Error",
							"message": "Please disable additional users to remove seats."
						});
					} else {
						isEditPlanVisible.value = !isEditPlanVisible.value;
						notification({
