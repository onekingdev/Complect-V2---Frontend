...
section.grid-6
	...
	template(v-if="!isBusiness")
		c-field.col-full(label="Description" type="textarea" placeholder="Description" required)
			...
			div
				div.experiences(v-for="(item, index) in userExperiences" :key="index")
					div.experiences-item
						...
						div.smallText {{ item.employer }} | {{ formatDate(item.startsAt) }} - {{ formatDate(item.endsAt) }}
...
export default {
	...
	setup () {
		...
		const openExperienceModal = (id) => modal({ "name": "cModalExperience", "id": id });
		...
		onMounted( () => {
			console.log(isBusiness.value)
			if (!isBusiness.value) getData();
		});
