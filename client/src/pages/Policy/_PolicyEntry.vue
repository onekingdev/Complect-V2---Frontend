	...
	template(#content)
		page-container(:badge="{title: policyDetails.status }" :title="policyDetails.name" type="document full-width")
			template(#controls)
				...
				c-button(title="Download" @click="exportPDF()"  v-if="policyDetails.status != 'archived' && policyDetails.role != 'Basic'")
				c-button-modal(title="Publish" modalTitle="New Policy" type="primary"  v-if="policyDetails.status != 'archived' && policyDetails.role != 'Basic'")
					template(#content)
						.col-1
			...
			template(#tabs v-if="policyDetails.status != 'archived'")
				...
				c-dropdown.action-button(title="Actions" type="transparent" v-if="policyDetails.role != 'Basic'")
