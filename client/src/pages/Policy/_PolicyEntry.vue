...
menu-container(:type="menuType")
	template(#leftmenu)
		...
		draggable(:list="documents" v-if="policyDetails.status != 'archived'")
		...
		page-container(:badge="{title: policyDetails.status }" :title="policyDetails.title" type="document full-width" :hasHamberger="hasHamberger")
			...
			template(#tabs v-if="policyDetails.status != 'archived'")
				...
				c-dropdown.action-button(title="Actions" type="transparent")
					c-button-modal(title="Archive" type="transparent" modalTitle="Archive Policy")
						template(#content)
							.col-1
			...
			template(#tabs  v-if="policyDetails.status == 'archived'")
				...
				c-dropdown.action-button(title="Actions" type="transparent")
					c-button-modal(title="Unarchive" type="transparent"  modalTitle="Unarchive Policy")
						template(#content)
							.col-1
								icon(name="warning" size="huge")
							.col-5
	...
	setup () {
		...
		const publishPolicy = async () => {
			try {
				...
				notification({
					"type": "success",
					"title": "Policy has been published."
				});
				toPoliciePage();
			} catch ( error ) {
				notification({
		...
		const saveDraft = async () => {
			try {
				...
				notification({
					"type": "success",
					"title": "Policy has been saved."
				});
			} catch ( error ) {
				notification({
					"type": "success",
					"title": "Policy has not been saved. Please try again."
				});
			}
		};
		const newPolicy = ref({
